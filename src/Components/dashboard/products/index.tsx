...
import {IProduct, ISupply} from "Data/Interfaces/Supply.ts";
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                header: "Price",
                size: 150,
                Cell: ({cell}) =>  <span>{cell.getValue()} <span  className='fw-bolder' style={{fontSize: '10px'}}> FCFA</span></span>,
                muiFilterTextFieldProps: () => ({
                    inputProps: { placeHolder: "filter" },
                }),
            },
            ...
            },
            {
                ...
                header: "Suppliers",
                size: 150,
                Cell: ({cell}) =>  <span>{cell.getValue().map(supply => supply.name).join('-')}</span>,
    ...
    const mrTable = useMaterialReactTable({
        ...
        renderTopToolbarCustomActions: () => (
            <Box sx={{ display: "flex", gap: "1rem", p: "4px" }}>
                {UtilMethods.getHabilitations(authorizations, 'article').canCreate && <button onClick={() => {
                    context.togglePageLoading(true)
                    dispatch(setActivePage({
                        page: Pages.ARTICLE,
                        param: {
                ...
                {UtilMethods.getHabilitations(authorizations, 'article').canExport && <button type='button' className='btn btn-outline-primary' style={{ marginLeft: '12px' }}>
